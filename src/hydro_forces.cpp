...
ForceFunc6d::ForceFunc6d(std::shared_ptr<ChBody> object, TestHydro* user_all_forces) : ForceFunc6d() {
	...
	if (all_hydro_forces == NULL) { std::cout << "all hydro forces null " << std::endl; }
	SetForce();
	SetTorque();
...
void ForceFunc6d::SetForce() {
	if (chrono_force == NULL || body == NULL) { std::cout << "set force null issue" << std::endl; }
...
void ForceFunc6d::SetTorque() {
	if (chrono_torque == NULL || body == NULL) { std::cout << "set torque null issue" << std::endl; }
...
TestHydro::TestHydro(std::vector<std::shared_ptr<ChBody>> user_bodies, std::string h5_file_name,
	...
	for (int b = 0; b < num_bodies; b++) { force_per_body.emplace_back(bodies[b], this); }
...
std::vector<double> TestHydro::ComputeForceHydrostatics() {
	...
	for (int b = 0; b < num_bodies; b++) {
		...
		for (int dof = 0; dof < 6; dof++) { body_force_hydrostatic[dof] += force_offset[dof]; }
...
std::vector<double> TestHydro::ComputeForceRadiationDampingConv() {
	...
	if (offset_rirf < -1 * size) { offset_rirf += size; }
...
#define TMP_S(row, step) tmp_s[(row) *size + (step)]
	...
	//thru all
	// bodies (0->11 for 2 bodies...) 				TMP_S(row, st) = TIMESERIES(row, col, st); 				sumVelHistoryAndRIRF
	// += TMP_S(row, st);
	//			}
	//		}
...
double TestHydro::coordinateFunc(int b, int i) {
	...
	if (bodies[0]->GetChTime() == prev_time) { return total_force[body_num_offset + i]; }
	...
	if (hydro_inputs.mode == WaveMode::noWaveCIC) {
		...
		for (int dof = 0; dof < total_dofs; dof++) { hydrostaticsCheck << force_hydrostatic[dof] << std::endl; }
		...
		for (int dof = 0; dof < total_dofs; dof++) { radiationCheck << force_radiation_damping[dof] << std::endl; }
		...
		for (int dof = 0; dof < total_dofs; dof++) { totalForceCheck << total_force[dof] << std::endl; }

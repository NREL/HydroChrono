#!/usr/bin/env python3
"""
Sphere Irregular Waves (State-Space Radiation) Regression Test Comparison

Compares sphere irregular wave results using state-space radiation approximation
...
if __name__ == '__main__':
    if len(sys.argv) != 3:
        print("Usage: python compare_sphere_irreg_waves_ss.py <reference_file> <results_file>")
        sys.exit(1)
    ...
    test_name = "Sphere Irregular Waves (State-Space)"
    y_label = "Heave (m)"
    executable_patterns = ["sphere_irreg_waves_ss_test", "sphere_irreg_waves_ss"]
    ...
    n1, n2, passed = run_comparison(
        ...
        status_name="sphere_irreg_waves_ss"
    )

...
% Fixed Constraint
constraint(1)= constraintClass('Fixed');  % Initialize ConstraintClass for Constraint1
constraint(1).bodies = [body(2)];

% Rotational PTO
constraint(1) = constraintClass('Rotational');                      % Initialize ptoClass for PTO1
constraint(1).stiffness = 0;                           % PTO Stiffness Coeff [Nm/rad]
constraint(1).damping = 0;                         % PTO Damping Coeff [Nsm/rad]
constraint(1).location = [0 0 -8.9];                   % PTO Location [m]
constraint(1).bodies = [body(1), body(2)];
constraint(1).attachments = [[0, 0, -8.9], [0, 0, -8.9]]; % PTO attachment points (global frame)